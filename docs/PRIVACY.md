...
- exact mathematical tool artifacts under `verification-artifacts`;
...
- titles and bounded chunks retrieved from imported PDF, DOCX, and text-like sources;
...
PDF, DOCX, text, Markdown, and LaTeX files selected by the user are copied locally. The app extracts text and stores deterministic chunks for local retrieval. Original PDF/DOCX bytes are not automatically uploaded, and image-only PDFs are not OCRed. Bounded retrieved excerpts can be sent as provider context during research or source-aware chat, so do not import material you are not permitted to process.
...
4. Delete `%APPDATA%\math-research-agent` to remove projects, settings, encrypted credentials, imported documents, tool workspaces, verification artifacts, and logs for the current Windows user.
