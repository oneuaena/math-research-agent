...
export class ResourceBudgetService {
  ...
  current(projectId: string): ResourceBudgetRecord {
    ...
    const created: ResourceBudgetRecord = { id: randomUUID(), projectId, status: 'ACTIVE', limits: { maxEvaluations: 1_000_000, maxWorkers: 32, maxProofAttempts: 128, maxToolSeconds: settings.maxToolSeconds }, used: { evaluations: 0, proofAttempts: 0, toolSeconds: 0 }, createdAt: now, updatedAt: now };
