...
import type { DiscoveryConfig, DiscoveryRun, DiscoverySpecificationInput } from '../shared/types';
import { useAppStore } from '../store';

const example = JSON.stringify({
  specification: {
    representation: { kind: 'SET', dimensions: { universeSize: 24, length: 7 }, schemaVersion: 1 },
    evaluator: {
      version: 1,
      constraints: [{ kind: 'forbidden-tuples', arity: 2, tuples: [[0, 1], [2, 3], [4, 5], [6, 7]] }, { kind: 'cardinality', target: 7 }],
      objectives: [{ name: 'violations', direction: 'minimize', metric: 'violations' }, { name: 'spread', direction: 'maximize', metric: 'spread' }], aggregation: 'pareto',
    },
    semanticScope: 'Finite set construction; results are evaluator-scoped candidates, not a theorem.',
  },
  config: { populationSize: 64, generations: 80, workerCount: 4, seed: 71, mutationRate: 0.18, archiveLimit: 48, strategy: 'evolutionary', evaluationBudget: 5120 },
}, null, 2);
...
export function DiscoveryLab() {
  ...
  const start = async () => {
    ...
    try {
      const result = await window.research.discovery.start(snapshot.project.id, JSON.parse(source) as { specification: DiscoverySpecificationInput; config: DiscoveryConfig });
  ...
  return <div className="discovery-view">
    <header className="view-toolbar"><div><h1>{language === 'zh' ? '构造发现' : 'Construction discovery'}</h1><span>{language === 'zh' ? '声明式候选 DSL · 可审计 evaluator · Pareto 归档' : 'Declarative candidate DSL · auditable evaluator · Pareto archive'}</span></div></header>
    <div className="discovery-content">
      <section className="discovery-card discovery-intro"><Sparkles size={20} /><div><h2>{language === 'zh' ? '受限的有限构造搜索' : 'Constrained finite-construction search'}</h2><p>{language === 'zh' ? '接受集合、图、超图、矩阵、向量与表达式树等数据表示。evaluator 是版本化声明式 DSL，绝不执行模型或用户提供的代码。结果是候选与计算记录，不是数学证明。' : 'Accepts data representations including sets, graphs, hypergraphs, matrices, vectors, and expression trees. The versioned evaluator DSL never executes model or user code. Results are candidates and computation records, not proofs.'}</p></div></section>
      <section className="discovery-card"><div className="discovery-section-head"><div><h2>{language === 'zh' ? '问题与预算' : 'Problem and budget'}</h2><p>{language === 'zh' ? '索引从 0 开始；worker 数量限制为 1–32；不通过验证的规格不会启动。' : 'Indices are zero-based; worker count is capped at 1–32; invalid specifications never start.'}</p></div><button className="button secondary" disabled={busy} onClick={() => setSource(example)}><RotateCcw size={14} />{language === 'zh' ? '载入格式' : 'Load format'}</button></div>
