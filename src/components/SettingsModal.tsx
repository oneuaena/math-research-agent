  ...
  return <Modal title={t(language, 'settings')} onClose={onClose} wide>
    <form className="settings-form" onSubmit={save}>
      ...
      <section><h3>{t(language, 'runtime')}</h3><div className="settings-grid">
        ...
        <label className="field"><span>{language === 'zh' ? '自主任务总时限 · 小时' : 'Autonomous job limit · hours'}</span><input type="number" min={1} max={720} value={settings.maxAutonomousHours} onChange={(e) => update('maxAutonomousHours', Number(e.target.value))} /></label>
        <label className="field"><span>{language === 'zh' ? '自主任务总 Token 上限' : 'Autonomous token limit'}</span><input type="number" min={10000} max={100000000} step={10000} value={settings.maxTotalTokens} onChange={(e) => update('maxTotalTokens', Number(e.target.value))} /></label>
