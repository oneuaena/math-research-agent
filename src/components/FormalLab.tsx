...
export function FormalLab() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const checked = await window.research.tools.run({ projectId: snapshot.project.id, name: 'lean_check', purpose: item.purpose, input: { code } });
  ...
  return <div className="formal-lab">
    ...
    <section className="formal-notice"><BookOpenCheck size={18} /><div><strong>Complete proofs only</strong><span>Kernel acceptance checks the submitted Lean theorem. It does not by itself prove that the formal statement matches the natural-language target.</span></div></section>
    ...
    <div className="formal-actions"><button className="button primary" disabled={running || gaps.length > 0} onClick={() => void run()}>{running ? 'Checking…' : 'Run Lean kernel check'}</button><span>Uses a project-local Mathlib v4.32.0 dependency and writes an audit artifact for every run.</span></div>
